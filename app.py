...
def calculate_iv(Jph_mA, J0_mA, n, Rs, Rsh, T, J_common):
    ...
    # Jsc bei V=0
    try:
        sol = root_scalar(lambda J: diode_equation_V(0.0, J/1000.0, cell), bracket=[0, Jph_mA*2], method="bisect")
        Jsc = sol.root
    except:
        Jsc = Jph_mA
...
Jsc_t = calculate_Jsc_tandem(cell1, cell2)
FF_t = P_mpp_t / (Jsc_t * Voc_tandem) if Voc_tandem * Jsc_t != 0 else 0
PCE_t = P_mpp_t
...
fig1.add_shape(type="line", x0=0, x1=0, y0=min(J_common), y1=max(J_common), line=dict(color="gray", dash="dash"))
fig1.add_shape(type="line", x0=min(V_tandem), x1=max(V_tandem), y0=0, y1=0, line=dict(color="gray", dash="dash"))
